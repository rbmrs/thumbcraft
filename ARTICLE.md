...
So Thumby stores text placement relative to the image dimensions, not in absolute preview pixels. The spot you drag to is recorded as a relationship to the image itself. No matter how large the original is, the exported PNG matches the layout you saw, rendered at full resolution. What you place is what you get, at full crispness, so the drag-to-place loop stays fast: you never export, check, and come back to fix a placement that drifted.
...
If you need layers, vector editing, or complex compositing, the big tools are still the right call. Thumby is for the last-hurdle case: image in, word on, placed by hand, exported as a PNG. It is open-source on GitHub: run it with `swift run`, or use the included package script to build a native `.app`.
